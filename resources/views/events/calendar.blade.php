...
<div class="py-6 px-4 max-w-5xl mx-auto">
    ...
    <form method="GET" action="{{ route('calendar.index') }}" class="mb-4 flex gap-6 items-center">
        ...
        <label class="flex items-center gap-2">
            <input type="radio" name="filter" value="all" {{ $filter === 'all' ? 
                'checked' : '' }} onchange="this.form.submit()">
            <span>Show All Events</span>
        </label>
        <label class="flex items-center gap-2">
            <input type="radio" name="filter" value="fav" {{ $filter === 'fav' ? 
                'checked' : '' }} onchange="this.form.submit()">
        ...
        <div class="grid grid-cols-7 gap-2">
            @foreach($dates as $date)
                @php
                    ...
                    $hasFav  = !$isBlank && collect($evs)->contains(fn($e) => 
                    $favIds->contains($e->id));
                @endphp

                <div class="h-24 rounded-xl border flex flex-col p-2 relative
                    {{ $isBlank ? 'bg-gray-50 border-gray-100' : ($date === $today ? 
                     'bg-blue-50 border-blue-300' : 'border-gray-200') }}">
                        ...
                        <div class="mt-2 space-y-1 overflow-y-auto max-h-16">
                            @foreach($evs as $e)
                                <a href="{{ route('events.show', $e->id) }}"
                                   ...
                                   {{ $favIds->contains($e->id) ? 'bg-yellow-100 text-yellow-800 
                                    hover:bg-yellow-200' : 'bg-gray-100 text-gray-700 hover:bg-blue-200 
    ...
    @php
        $todayFavs = $allUpcomingFavs->filter(fn($e) => \Carbon\Carbon::parse($e->event_date)
        ->toDateString() === $today);
    @endphp
    ...
    @if($todayFavs->count())
        ...
        <div class="space-y-4">
            @foreach($todayFavs as $event)
                <div class="bg-black text-white p-6 rounded-2xl shadow-lg flex justify-between 
                items-center w-full">
                    <div class="flex flex-col">
                        <h3 class="font-bold text-lg">{{ $event->title }}</h3>
                        <a href="{{ route('events.show', $event->id) }}" class="text-xs 
    ...
    @if($futureFavs->count())
        ...
        <div class="space-y-4">
            @foreach($futureFavs as $event)
                @php $eventDate = \Carbon\Carbon::parse($event->event_date)->format('Y-m-d H:i:s'); 
                @endphp
                <div class="bg-white border p-4 rounded-xl shadow-sm flex justify-between 
                items-center w-full">
                    <div class="flex flex-col">
                        <h3 class="font-bold text-lg text-gray-800">{{ $event->title }}</h3>
                        <a href="{{ route('events.show', $event->id) }}" class="text-xs 
                        text-blue-600 hover:text-blue-800">View Details →</a>
                    </div>
                    <div class="text-center"
                         x-data="{
                        ...
                        <p class="text-lg font-bold text-gray-800" x-text="days + 'd '
                         + hours + 'h ' + minutes + 
                        'm ' + seconds + 's'"></p>
                    </div>
                </div>
    ...
    <div class="flex justify-end mt-6">
        <a href="{{ route('calendar.export') }}" class="px-4 py-2 rounded-xl
